    ...
    const getFilteredData = () => {
        ...
        switch (timeRange) {
            ...
            case '5Y':
                ...
                break;
            case 'Max':
                startDate = new Date('2000-01-01'); // User requested Since 2000 (BTC starts later, effectively Max)
                break;
            default:
                startDate.setFullYear(now.getFullYear() - 1);
        }

        return data.filter(item => new Date(item.date) >= startDate && item.btc !== null && item.btc !== undefined);
    };

    const chartData = getFilteredData();
    const latestValue = chartData.length > 0 ? chartData[chartData.length - 1].btc : 0;
