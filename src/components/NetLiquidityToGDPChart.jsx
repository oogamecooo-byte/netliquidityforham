...
const NetLiquidityToGDPChart = ({ data }) => {
    ...
    const getFilteredData = () => {
        ...
        return data.filter(item => new Date(item.date) >= startDate && item.liquidityToGdpRatio !== null);
    };
        ...
        <div className="bg-slate-950 p-6 rounded-xl border border-slate-800 shadow-xl">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1">Net Liquidity as a percentage of Gross Domestic Product</p>
                </div>

                <div className="flex flex-col items-end gap-2">
                    <div className="text-2xl font-bold text-slate-100">
                        {filteredData.length > 0 ? `${filteredData[filteredData.length - 1].liquidityToGdpRatio?.toFixed(2)}%` : '0.00%'}
                    </div>
...
export default NetLiquidityToGDPChart;
